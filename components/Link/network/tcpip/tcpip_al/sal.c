...
typedef struct
{
    ...
    int                     fd;
    void                   *sock;          ///< returned by the implement functions
}tag_sock_cb;
...
int tcpip_sal_init( int socknum)
{
    ...
    s_sal_cb.sock_cb_tab = osal_mem_malloc(socknum*sizeof(void *));
    if(NULL == s_sal_cb.sock_cb_tab)
    {
        goto EXIT_MEM_ERR;
    }
...
static void * __sal_sockcb_malloc()
{
    ...
    sockcb = osal_mem_malloc(sizeof(tag_sock_cb));
...
int sal_accept(int sockfd,struct sockaddr *addr,socklen_t *addrlen)
{
    ...
    if(NULL != sockcb_client)
    {
        ...
        {
            __sal_sockcb_free(sockcb_client);
        }
    }
