...
int mqtt_encode_fixhead(unsigned char *buf, unsigned char type, unsigned char dup, unsigned char qos,
    unsigned char retain, int remaining_len)
{
    int len = 1;
...
int mqtt_decode_fixhead(unsigned char *buf, unsigned char *type, unsigned char *dup, unsigned char *qos,
    unsigned char *retain, int *remaining_len)
{
    int len = 1;
...
int mqtt_encode_connect(unsigned char *buf, int buf_len, mqtt_connect_opt_t *options)
{
    ...
    if (options->connect_head.mqtt_connect_flag_u.bits.will_flag && options->connect_payload.will_topic &&
    ...
    memcpy(vhead_buf, &(options->connect_head),
        sizeof(options->connect_head) - sizeof(options->connect_head.keep_alive));
    mqtt_encode_num(vhead_buf + sizeof(options->connect_head) - sizeof(options->connect_head.keep_alive),
        options->connect_head.keep_alive);
...
int mqtt_encode_publish(unsigned char *buf, int buf_len, mqtt_publish_opt_t *options)
{
    ...
    /* Encode fix header */
    len =
        mqtt_encode_fixhead(buf, MQTT_PACKET_TYPE_PUBLISH, options->dup, options->qos, options->retain, remaining_len);
...
int mqtt_encode_puback(unsigned char *buf, int buf_len, mqtt_puback_opt_t *options)
{
    ...
    len = mqtt_encode_fixhead(buf, options->type, 0, (options->type == MQTT_PACKET_TYPE_PUBREL) ? 1 : 0, 0,
        remaining_len);
    vhead_buf = buf + len;
