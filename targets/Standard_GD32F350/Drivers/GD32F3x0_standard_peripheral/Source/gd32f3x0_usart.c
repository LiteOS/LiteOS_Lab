...
/*!
    ...
    \param[in]  usart_periph: USARTx(x=0,1)
    \param[in]  usartaddr: 0x00-0xFF, address of USART terminal
    \param[out] none
    \retval     none
*/
void usart_address_config(uint32_t usart_periph, uint8_t usartaddr)
{
    ...
    USART_CTL1(usart_periph) |= (USART_CTL1_ADDR & (((uint32_t)usartaddr) << 24));
}
...
/*!
    ...
    \param[in]  usart_periph: USARTx(x=0,1)
    \param[in]  int_flag: interrupt type
      ...
      \arg        USART_INT_RFFIE:  receive FIFO full interrupt enable
    \param[out] none
    \retval     none
*/
...
/*!
    ...
    \param[in]  flag: USART flags, refer to usart_flag_enum
      ...
      \arg        USART_FLAG_IDLEF: idle line detected flag
      ...
      \arg        USART_FLAG_ABDF:  auto baudrate detection flag
      \arg        USART_FLAG_BSY:   busy flag
      \arg        USART_FLAG_AMF:   address match flag
      \arg        USART_FLAG_SBF:   send break flag
      \arg        USART_FLAG_RWU:   receiver wakeup from mute mode.
      \arg        USART_FLAG_WUF:   wakeup from deep-sleep mode flag
...
/*!
    ...
    \param[in]  int_flag: interrupt and flag type, refer to usart_interrupt_flag_enum
      \arg        USART_INT_EBIE_EBF:    end of block interrupt and flag
      \arg        USART_INT_RTIE_RTF:    receiver timeout interrupt and flag
      \arg        USART_INT_AMIE_AMF :   address match interrupt and flag 
      \arg        USART_INT_PERRIE_PERR: parity error interrupt and flag 
      \arg        USART_INT_TBEIE_TBE:   transmitter buffer empty interrupt and flag 
      \arg        USART_INT_TCIE_TC:     transmission complete interrupt and flag
      \arg        USART_INT_RBNEIE_RBNE: read data buffer not empty interrupt and flag
      \arg        USART_INT_RBNEIE_ORERR:read data buffer not empty interrupt and overrun error flag
      \arg        USART_INT_IDLEIE_IDLEF:IDLE line detected interrupt and flag
      \arg        USART_INT_LBDIE_LBDF:  LIN break detected interrupt and flag 
      \arg        USART_INT_WUIE_WUF:    wakeup from deep-sleep mode interrupt and flag
      \arg        USART_INT_CTSIE_CTSF:  CTS interrupt and flag
      \arg        USART_INT_ERRIE_NERR:  error interrupt and noise error flag
      \arg        USART_INT_ERRIE_ORERR: error interrupt and overrun error
      \arg        USART_INT_ERRIE_FERR:  error interrupt and frame error flag
      \arg        USART_INT_RFFIE_RFFINT:receive FIFO full interrupt and flag
    \param[out] none
    \retval     FlagStatus: SET or RESET
*/
...
/*!
    ...
    \param[in]  usart_periph: USARTx(x=0,1)
    \param[in]  flag: flag types refer to usart_flag_enum
      \arg        USART_FLAG_PERR: parity error flag
      \arg        USART_FLAG_FERR: frame error flag
      \arg        USART_FLAG_NERR: noise detected flag
      \arg        USART_FLAG_ORERR:overrun error flag
