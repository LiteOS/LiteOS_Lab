...
/* ============================================================
func name   :  neul_bc95_set_NNMI
discription : Set Power Saving Mode Status              
param       :  buf @ out param
return      : 0 mean ok, <0 not ok
Revision    : 
=============================================================== */
int neul_bc95_set_nnmi(void)
{
	  int ret = 0;
	  char *cmd = "AT+NNMI=0\r\n";
    char *str = NULL;		
...
int uart_data_read(char *buf, int maxrlen, int mode, int timeout)
{
    ...
    {
        //block mode
        retlen = los_dev_uart_read(LOS_STM32L431_UART2, buf, maxrlen, timeout);
    }
    else
    {
        //none block mode
        retlen = los_dev_uart_read(LOS_STM32L431_UART2, buf, maxrlen, 0);
    }
